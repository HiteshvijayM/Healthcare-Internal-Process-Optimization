...
**Created**: 2026-08-06
**Status**: Ratified — all governance blockers cleared (CHG-021); ready for planning
...
5. **Given** an exact re-send of a document already on file, **When** it arrives after the key-matching window has closed and against a case that is already closed, **Then** it is still flagged as a duplicate on document identity and held for adjudication, rather than being admitted as new work.
6. **Given** a later submission sharing the same sender, patient reference and requested service but carrying genuinely different content, **When** it arrives after the key-matching window has closed, **Then** it is not flagged on document identity and is admitted as new work.
7. **Given** a case record containing a contradictory or implausible value, **When** completeness checking runs, **Then** the conflict is named explicitly and the case is not allowed to advance on that value alone.
...
7. **Given** no on-call clinical coverage is configured for the period the acknowledgement would fall in, **When** a critical-condition signal is detected, **Then** a governance blocker is raised, the acknowledgement clock does not start, and no acknowledgement breach is recorded against an unstaffed period.
8. **Given** more than one required designation is absent, **When** the signal is evaluated, **Then** a single governance blocker is raised that names every absent designation rather than surfacing one and concealing the others.
9. **Given** a diagnostic input containing a concerning value that is **not** an entry in the critical-condition signal register, **When** detection runs, **Then** no critical-condition signal is raised, no escalation packet is prepared, and the assistant does not generalise from a similar register entry to treat the value as critical.
10. **Given** the critical-condition signal register is absent, empty, or its version cannot be resolved, **When** a case is evaluated, **Then** a governance blocker is raised, the case is held with provisional routing refused, and the result is not reported as "no critical condition found".
11. **Given** a register entry that resolves successfully, **When** a matching signal is detected, **Then** the recorded register version, the matched entry identifier and that entry's clinical owner are all captured on the escalation packet, so the basis of the match is auditable after the fact.
...
1. **Given** a case awaiting release, **When** clinical clearance and financial clearance are each completed by a separate authorized human, **Then** and only then does the case become eligible for release routing. *(AS-11)*
...
4. **Given** a case awaiting release, **When** financial clearance is recorded before clinical clearance, **Then** it is accepted rather than refused for ordering, and release remains blocked until clinical clearance is also recorded.
...
- **FR-014**: System MUST flag an arriving request as a probable duplicate when it matches an in-progress case on sender, patient reference and requested service within the approved duplicate-detection window — currently 72 hours from first receipt under P2 — independent of arrival channel.
- **FR-055**: System MUST additionally flag an arriving item as a duplicate whenever it is an exact re-send of a document already on file — matched on **document identity** rather than on the sender/patient/service key — **regardless of how much time has elapsed and regardless of whether the earlier case is still in progress or already closed** — and MUST hold it for adjudication under FR-015. Two documents have the same identity when either they carry the same immutable source-document identifier supplied by the arrival channel, or their normalized content is identical, where normalization removes transport-added material only (cover sheets, routing headers, arrival timestamps, channel watermarks and re-transmission banners) and alters no clinical or administrative content. A difference in arrival channel, file format or resolution MUST NOT by itself defeat an identity match, and a difference in any retained content MUST prevent one. The FR-014 window bounds key-based matching only; it MUST NOT suppress an identity-match detection.
...
- **FR-022**: System MUST track approval SLAs resolved per urgency class and service line from the approved policy table, defaulting to 2 business days for routine items, 4 hours for urgent items, and 30 minutes for acknowledgement of a critical escalation where a service line defines no override. System MUST record which SLA value was applied to each item so a breach can be audited against the value in force at the time.
- **FR-056**: The critical acknowledgement clock in FR-022 MUST start at the moment a critical-condition signal is detected under FR-057, not at dispatch, so that the clock measures elapsed clinical risk rather than elapsed administrative handling. System MUST confirm at that moment that on-call clinical coverage is configured for the period the acknowledgement falls in, and where no coverage is configured MUST raise a governance blocker under the precedence rule in FR-054 rather than starting a clock that no named human is accountable for answering.
...
- **FR-057**: System MUST detect critical-condition signals **only** by matching against the explicit critical-condition signal register that is human-curated, versioned, and change-controlled, in which every entry carries an identifier, a matching rule, and a recorded clinical owner. The register in force for this build is **`CCR-DEMO-v1`** (`docs/critical-condition-register.md`, policy P11). System MUST NOT infer, generalise, or extend a critical condition beyond the entries in that register. Where the register is absent, empty, or its version cannot be resolved, System MUST raise a governance blocker, MUST hold the case and MUST NOT permit provisional routing or any other progression under FR-010 until a valid register version resolves, and MUST NOT report the absence of a match as evidence that no critical condition is present. Where a case matches more than one register entry, System MUST produce exactly one escalation packet naming every matched signal identifier rather than one packet per match. System MUST record the register version in force against every escalation decision, per FR-045.
- **FR-024**: System MUST prepare an escalation packet within 30 seconds of detecting a critical-condition signal in test or diagnostic inputs, consistent with the draft-readiness bound in SC-003, and MUST present that packet for dispatch approval only once the outcome-precedence rule in FR-054 selects the approval outcome — that is, once the clinical recipient, the Escalation Dispatch Approver, an approved dispatch-approval deadline, and on-call clinical coverage are all designated *and* the packet satisfies the completeness requirement in FR-025. System MUST NOT dispatch it to the designated clinical authority without the recorded human approval required by FR-030 and FR-051.
...
- **FR-051**: System MUST require escalation-packet dispatch to be approved by the designated Escalation Dispatch Approver — **the Intake Coordinator, with the Team Lead as named alternate**, per the required-designations table in the approver role registry — who MUST NOT be that packet's designated clinical recipient, and MUST surface a pending dispatch as a non-suppressible alert that cannot be dismissed without a recorded approve or reject decision. Where no Escalation Dispatch Approver has been designated for the run, System MUST hold the packet and raise a governance blocker rather than dispatching it or defaulting to another role.
- **FR-052**: System MUST enforce the approved dispatch-approval deadline — **10 minutes under policy P10** — which MUST be strictly shorter than the critical acknowledgement SLA actually applied to that case and recorded under FR-022 (30 minutes where no service-line override is in force), so the remainder of that window stays available for clinical acknowledgement. System MUST record a breach when the deadline elapses, MUST NOT dispatch on breach without an approval, MUST escalate a breached dispatch approval to the named alternate approver, and MUST raise a governance blocker rather than adopting a default value where no such deadline has been approved, or where the approved deadline is not shorter than the applied acknowledgement SLA.
...
- **FR-033**: System MUST require both clinical clearance and financial clearance, each recorded by a separate authorized human, before a case becomes eligible for release routing. The two clearances are **order-independent**: System MUST accept either one first and MUST NOT refuse a clearance solely because the other is still outstanding. System MUST refuse release routing until both are recorded and MUST name whichever gate is outstanding as the blocker.
...
| AS-7 | Duplicate detected and flagged rather than reprocessed | US1 | FR-014, FR-015, FR-055 |
...
| AS-10 | Critical signal auto-prepares escalation to clinical authority, no clinical decision | US5 | FR-024, FR-025, FR-026, FR-027, FR-028, FR-051, FR-052, FR-053, FR-054, FR-056, FR-057 |
| AS-11 | Clinical clearance then finance clearance, then release eligibility (either order accepted — see Assumptions) | US6 | FR-033, FR-034, FR-030 |
...
- **Duplicate Flag**: A probable-duplicate marker linking a newly arrived request to an earlier case — which may be in progress or already closed — recording which rule matched (the windowed sender/patient/service key, or unbounded document identity), the matched key or document identifier, and the adjudication state.
...
- **Role**: An authority (not a person) that may act on a case — Intake Coordinator, the five role-scoped approvers, Clinical Authority, Finance Clearance Approver, Team Lead, Compliance Reviewer, Team Validation Lead. **Escalation Dispatch Approver** is a required *designation* over this registry rather than a new authority; it is held by the **Intake Coordinator** with the **Team Lead** as named alternate (ratified CHG-021).
- **Critical-Condition Signal Register**: The versioned, change-controlled set of registered critical signals (`CCR-DEMO-v1`), each entry carrying an identifier, a matching rule, and a named clinical owner. It is the exclusive source of critical-condition matches; nothing outside it is a critical condition, and its absence blocks rather than permits progression.
...
- **SC-003**: A draft is ready in under 30 seconds at the 95th percentile, computed by the nearest-rank method over every graded case in the run, measured from intake to draft ready, with the sample size reported alongside the figure and any case exceeding the bound listed individually with its cause. The graded population is every case admitted to the run, including cases that later block, so slow cases cannot be excluded by their outcome.

**Fewer errors**

- **SC-004**: Key details are extracted correctly for at least 85% of graded fields across the fixed sample document set. The graded denominator is the seven fields named in `graded_fields` in `data/sample/answer-key.json` across all 20 cases (n = 140); `supporting_notes` is extracted but not graded, being free narrative text with no single correct value. The denominator MUST be reported with the percentage.
...
- **SC-009**: 100% of duplicate submissions in the sample set are flagged, and no non-duplicate is falsely flagged. The sample set MUST include at least one exact re-send arriving after the key-matching window has closed (which must be flagged under FR-055) and at least one later submission sharing the sender/patient/service key but carrying genuinely different content (which must not be flagged under FR-055), so that the window boundary and the identity boundary are each exercised in both directions. **Grading MUST assert which matcher fired**, not merely that a flag was raised — a key-match-only implementation satisfies flag correctness completely while never invoking the identity matcher at all.

**Operational reliability**

- **SC-010**: Approval SLA compliance improves relative to the sequential baseline, measured as the proportion of approvals closed within the SLA value recorded as applied to each item under FR-022 — defaulting to 2 business days routine, 4 hours urgent and 30 minutes critical acknowledgement where the item's service line defines no override — so that a breach is always graded against the value in force at the time rather than against a global constant.
- **SC-011**: For 100% of critical-condition signals in the sample set the system produces exactly one outcome, selected in this precedence order: (1) a **governance blocker** where any required designation is absent — the clinical recipient (FR-028), the Escalation Dispatch Approver (FR-051), an approved dispatch-approval deadline (FR-052), or configured on-call clinical coverage (FR-056) — which takes precedence over any completeness finding, including a missing recipient that would otherwise read as a missing mandatory field, and which names every absent designation rather than only the first; (2) a **completeness blocker** where all four designations are present but the packet is missing a mandatory field (FR-025, FR-026); (3) a **dispatch approval** carrying a complete packet where all four designations are present and no mandatory field is missing. 100% of approved packets are routed to the designated clinical authority, zero partial packets are sent, and a rejected packet remains undispatched with its rationale recorded.
...
Reasonable defaults chosen where the feature request did not specify a value. Each is the most conservative option consistent with the non-negotiable constraints. All items previously flagged for ratification were ratified under **CHG-021** and are now approved values.

- **Numeric policy values are adopted from the project's already-approved policy table** (`feature.md` §5.4, P1–P11) rather than invented, because those values were decided and change-logged before this specification and reopening them would contradict an approved decision.
...
- **"A short time of case arrival"** for draft readiness means under 30 seconds from intake to draft ready (`feature.md` §7). The specification measures that bound at the 95th percentile (SC-003) rather than per case, since an unqualified per-case bound is unfalsifiable without a stated population and one slow outlier should not fail an entire build. **Ratified under CHG-021** — `feature.md` §7 now states the bound as a p95 with the sample size reported and outliers itemised.
- **Approval SLA targets** are 2 business days routine / 4 hours urgent / 30 minutes critical acknowledgement, warning at 80% elapsed (P4, P5). The specification resolves these per urgency class *and service line*, treating the three approved figures as defaults where a service line defines no override, and requires the applied value to be recorded so a breach is audited against the value in force (FR-022). **Ratified under CHG-021** — P4 now carries the per-service-line resolution model, in which an override may only shorten a default; lengthening one requires Compliance Reviewer approval. Where a service line registers no override the approved global default applies.
- **The duplicate window** is 72 hours with a sender + patient reference + requested service match key (P2), and a match results in a flag held for human adjudication rather than any automated resolution. That window bounds *key-based* matching only — matching on the composite sender/patient/service key, which can match documents whose contents differ. An exact re-send of a document already on file is matched on **document identity** and flagged regardless of elapsed time and regardless of whether the earlier case is closed (FR-055), which closes the gap where a clinical re-fax arriving on day five would otherwise pass as new work. **Ratified under CHG-021** — P2 now defines both matches: the 72-hour key match and the unbounded identity match. The 72-hour figure is a policy-table parameter rather than a constant, so it can be retuned without a spec change.
- **The set of "critical conditions"** is a fixed, human-curated, versioned and change-controlled register (FR-057, policy P11). The assistant matches against that register and never infers new critical conditions on its own; where the register is absent, empty, or its version cannot be resolved it raises a governance blocker and holds the case rather than reporting "no critical condition found". **Resolved under CHG-021, fully covered under CHG-024** — the register exists as `docs/critical-condition-register.md`, version `CCR-DEMO-v1`, with three entries: a radiologist critical result flag (CCS-001), an explicit clinician escalation request (CCS-002), and a laboratory critical-value notification (CCS-003). CCS-001 and CCS-002 were seeded in `SYN-CASESET-v1`; CCS-003 had no fixture until `SYN-CASESET-v2` added CASE-023, so every entry is now exercised in the positive direction and the harness computes that coverage from the register rather than assuming it. Every entry matches a marker a qualified human already applied; no entry describes a clinical state. Administrative urgency (`Urgent`/`STAT`) is explicitly **not** a registered signal. The register is demo-scoped: populating it for real clinical use remains a clinical deliverable and is a hard precondition of processing real data, alongside the P8 retention gate.
...
- **Sample data** is `SYN-CASESET-v2` (23 hand-authored synthetic case documents with a JSON answer key in `data/`; v2 extends v1 by three fixtures and changes nothing else), with provenance recorded per constitution §3. No external dataset is adopted and no real patient data is used at any point.
...
- **Escalation-packet dispatch requires recorded approval by a designated Escalation Dispatch Approver**, who must not be the packet's own clinical recipient, and where no such approver is designated the packet is held under a governance blocker rather than dispatched. The source request described escalation as automatic; the non-negotiable constraint that every outbound action carries a recorded human approval takes precedence, so automation was limited to packet preparation plus a non-suppressible approval prompt. **Ratified under CHG-021** — the designation is held by the **Intake Coordinator**, with the **Team Lead** as named alternate, recorded in the required-designations table of the approver role registry. Dispatch approval is an administrative completeness-and-addressing check rather than a clinical judgement (the packet asserts none, per FR-027), the Intake Coordinator already owns completeness under FR-026, and separation of duty from the clinical recipient therefore holds by construction.
- **The dispatch-approval deadline is 10 minutes** (policy **P10**, ratified under CHG-021). It must always be strictly shorter than the critical acknowledgement SLA applied to the case, so the clinician retains the majority of the window; at the 30-minute default that leaves 20 minutes for clinical response. FR-052 still raises a governance blocker rather than adopting a default should the approved value ever be removed or become non-shorter than the applied SLA.
- **The critical acknowledgement clock starts at detection**, not at dispatch (FR-056), so the 30-minute window measures elapsed clinical risk rather than elapsed administrative handling. This is what makes the dispatch-approval deadline meaningful as a sub-interval of that window, and it is why absent on-call coverage is treated as a governance blocker evaluated alongside the other missing designations under FR-054 rather than as a separate post-dispatch condition.
- **Clearance ordering is deliberately order-independent** (FR-033). The source acceptance scenario AS-11 reads "clinical clearance then finance clearance", but the property that actually protects the patient is the *conjunction* of both gates, not their sequence — financial clearance cannot cause clinical harm, so mandating order adds latency without adding safety. Order-independence is a superset of the source scenario, so AS-11 still passes exactly as written. Clinical-first is retained as a non-binding efficiency preference, since it avoids financial work on a case that clinical review will stop.
- **Completion-task ownership** is determined by an inspectable field-to-role mapping over the approver role registry, defaulting to the Intake Coordinator where a field has no mapped owner. **Adopted default mapping** (CHG-021), aligned to the five approved routing queues so no new taxonomy is introduced: insurance fields → Insurance Approver, operations → Operations Approver, diagnostics → Diagnostics Approver, legal → Legal Approver, finance → Finance Approver, everything unmapped → Intake Coordinator. The mapping is a versioned policy artifact so operations can retune it without a spec change.
